...
3. Repository name 填 `Golden-Dolphin`。
4. Visibility 先选 Private。
5. 不要勾选初始化 README、.gitignore、license。
6. 创建后，在本地执行：

```bash
git remote add origin git@github.com:forest27xx/Golden-Dolphin.git
...
```bash
gh repo create forest27xx/Golden-Dolphin --private --source . --remote origin --push
git push -u origin dev
```
